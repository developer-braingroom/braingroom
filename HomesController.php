...
class HomesController extends AppController {
  ...
  /************** added by vineet ********/
        ...
        //$segment = $this->ClassSegment->find('all',array('conditions' => array('ClassSegment.id IN'=> $class_detail['VendorClasse']['segment_id'])));
         $segment = $this->ClassSegment->query('select * from `bg_class_segments` where `id` IN ('.$class_detail['VendorClasse']['segment_id'].')');
  ...
  $this->set('seg_id',$seg_id);
    ...
    else
    {
      ...
      $this->paginate=array('limit'=>5,
                      'conditions'=>array('VendorClasse.category_id'=>$cat_id,'FIND_IN_SET(\''. $seg_id.'\',VendorClasse.segment_id)'));
              $res_class = $this->paginate('VendorClasse');
        $this->set('allclass',$res_class);
        $this->set('cat_id',$cat_id);
        $this->set('seg_id',$seg_id);
...
public function findsubCat(){
       ...
       $cat_sub=$this->ClassSegment->find('all',array('conditions'=>array('category_id'=>$cat_id))); 
         ...
         echo '<select class="form-control reg_input" name="segment_id[]" id="segment_id" multiple="multiple">
          ...
          echo '</select><div id="s1" class="error_msg">&nbsp;&nbsp;</div>'; 
}
                ...
                <html>
                    ...
                    <body>
                        <div style="border-radius: 6px;background-color: rgba(255,255,255,0.3);padding: 10px;width: 81%;margin-left:20px;">
                            ...
                            <span style="font-size:11px;color:#8a8a8a;line-height:100%">Copyright Â© 2013 braingroom.com All rights reserved.</span>
                        </div>        
                    </body>
                </html>');
                $sendgrid->send($email);
...
If (isset($_POST["additionalCharges"])) {
 ...
 public function catalogDetail(){
          ...
          $cataloglist=$this->VendorClasse->query("select * FROM bg_vendor_classes,bg_request_catalogs where bg_vendor_classes.user_id=bg_request_catalogs.vendor_id and catalogue_cat_id=$catalog_id");
...
public function classes_onchange_bysegment($cat_id = NULL, $segment_id = NULL){
